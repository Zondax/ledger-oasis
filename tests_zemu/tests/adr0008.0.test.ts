...
describe('Standard-Adr0008-0', function () {
  ...
  test.concurrent.each(models)('main menu', async function (m) {
    ...
    try {
      ...
      await sim.start({ ...defaultOptions, model: m.name })
      await sim.navigateAndCompareSnapshots('.', `${m.prefix.toLowerCase()}-adr0008-0-mainmenu`, mainmenuNavigation.schedule)
    } finally {
      await sim.close()
    }
  })
  ...
  test.concurrent.each(models)('sign amend schedule', async function (m) {
    ...
    try {
      ...
      await sim.waitUntilScreenIsNot(sim.getMainMenuSnapshot(), 20000)
      await sim.compareSnapshotsAndApprove('.', `${m.prefix.toLowerCase()}-adr0008-0-sign_amend`, true)
  ...
  test.concurrent.each(models)('sign amend schedule - issue #130', async function (m) {
    ...
    try {
      ...
      await sim.waitUntilScreenIsNot(sim.getMainMenuSnapshot(), 20000)
      await sim.compareSnapshotsAndApprove('.', `${m.prefix.toLowerCase()}-adr0008-0-sign_amend_issue_130`, true)
