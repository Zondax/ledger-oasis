...
describe('Standard-Adr0008-5', function () {
  ...
  test.each(models)('main menu', async function (m) {
    ...
    try {
      await sim.start({...defaultOptions, model: m.name,});
      await sim.navigateAndCompareSnapshots('.', `${m.prefix.toLowerCase()}-adr0008-5-mainmenu`, [1, 0, 0, 4, -5])
    } finally {
      await sim.close();
    }
  });
  ...
  test.each(models)('sign amend schedule', async function (m) {
    ...
    try {
      ...
      await sim.waitUntilScreenIsNot(sim.getMainMenuSnapshot(), 20000);
      await sim.compareSnapshotsAndAccept(".", `${m.prefix.toLowerCase()}-adr0008-5-sign_amend`, m.name === "nanos" ? 29 : 30);
  ...
  test.each(models)('sign amend schedule - issue #130', async function (m) {
    ...
    try {
      ...
      await sim.waitUntilScreenIsNot(sim.getMainMenuSnapshot(), 20000);
      await sim.compareSnapshotsAndAccept(".", `${m.prefix.toLowerCase()}-adr0008-5-sign_amend_issue_130`, m.name === "nanos" ? 20 : 21);
