...
static parser_error_t readBigInt(parser_context_t *ctx,
                                 eth_big_int_t *big_int) {
...
static parser_error_t parse_legacy_tx(parser_context_t *ctx, eth_tx_t *tx_obj) {
  ...
  // parse data
  CHECK_PARSER_ERR(
      parse_field(ctx, &(tx_obj->legacy.data_at), &tx_obj->legacy.dataLen));
  // two cases:
  ...
  if (r_len == 1 && s_len == 1 &&
      (ctx->buffer[r_offset] | ctx->buffer[s_offset])) {
