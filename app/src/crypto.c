...
zxerr_t crypto_extractPublicKeySr25519(uint8_t *pubKey, uint16_t pubKeyLen) {
    ...
    // Generate keys
    CATCH_CXERROR(
        os_derive_bip32_with_seed_no_throw(mode, CX_CURVE_Ed25519, hdPath, hdPathLen, privateKeyData, NULL, NULL, 0));
...
zxerr_t crypto_signSecp256k1(uint8_t *output, uint16_t outputLen, const uint8_t *message, uint16_t messageLen,
                             uint16_t *sigSize) {
    ...
    *sigSize = 0;
    ...
    CATCH_CXERROR(cx_ecdsa_sign_no_throw(&cx_privateKey, CX_RND_RFC6979 | CX_LAST, CX_SHA256, message, messageLen,
                                         signature_object->der_signature, &maxSignatureLen, &info));
...
zxerr_t crypto_signEd25519(uint8_t *output, uint16_t outputLen, const uint8_t *message, uint16_t messageLen,
                           uint16_t *sigSize) {
    if (output == NULL || message == NULL || sigSize == NULL || outputLen < ED25519_SIGNATURE_SIZE ||
        messageLen != CX_SHA256_SIZE) {
    ...
    *sigSize = 0;
    ...
    CATCH_CXERROR(cx_ecfp_init_private_key_no_throw(CX_CURVE_Ed25519, privateKeyData, 32, &cx_privateKey));
    CATCH_CXERROR(cx_eddsa_sign_no_throw(&cx_privateKey, CX_SHA512, message, messageLen, output, outputLen));
...
zxerr_t crypto_sign_sr25519(uint8_t *output, uint16_t outputLen, const uint8_t *data, size_t len, const uint8_t *ctx,
    ...
    if (output == NULL || data == NULL || sigSize == NULL || ctx == NULL || outputLen < SIG_LEN) {
        return zxerr_unknown;
    }
    ...
    *sigSize = 0;
    ...
    zxerr_t error = zxerr_unknown;
    const int mode = (hdPathLen == HDPATH_LEN_ADR0008) ? HDW_ED25519_SLIP10 : HDW_NORMAL;
    CATCH_CXERROR(os_derive_bip32_with_seed_no_throw(mode, CX_CURVE_Ed25519, hdPath, hdPathLen, sk, NULL, NULL, 0));
...
zxerr_t _sign(uint8_t *output, uint16_t outputLen, const uint8_t *message, uint16_t messageLen, uint16_t *sigSize,
    ...
    *sigSize = 0;
    ...
    zxerr_t error = zxerr_unknown;
    CATCH_CXERROR(
        os_derive_bip32_with_seed_no_throw(HDW_NORMAL, CX_CURVE_256K1, hdPath, hdPathLen, privateKeyData, NULL, NULL, 0));
...
zxerr_t crypto_sign_eth(uint8_t *buffer, uint16_t signatureMaxlen, const uint8_t *message, uint16_t messageLen,
    ...
    if (err != zxerr_ok) return zxerr_invalid_crypto_settings;
...
uint16_t crypto_encodeAddress(char *addr_out, uint16_t addr_out_max, uint8_t *pubkey, address_kind_e kind) {
    ...
    SHA512_256_with_context_version((uint8_t *)context, strnlen(context, MAX_CONTEXT_SIZE), COIN_ADDRESS_VERSION, pubkey,
                                    PK_LEN_ED25519, tmp.pkHash);

    //  and encode as bech32
    const zxerr_t err = bech32EncodeFromBytes(addr_out, addr_out_max, COIN_HRP, tmp.address,
                                              sizeof_field(tmp_address_t, address), 1, BECH32_ENCODING_BECH32);
...
static uint16_t crypto_encodeEthereumAddress(char *addr_out, uint16_t addr_out_max, uint8_t *pubkey) {
    ...
    keccak(hash, KECCAK256_HASH_LEN, pubkey + 1, HASH_SIZE);
...
zxerr_t crypto_fillEthAddress(uint8_t *buffer, uint16_t buffer_len, uint16_t *addrLen) {
    ...
    CHECK_ZXERR(
        crypto_extractPublicKeySecp256k1(&answer->publicKey[1], sizeof_field(answer_eth_t, publicKey) - 1, &chain_code))
